...
import { sendOTP, verifyOTP, createUserProfile } from '@/api/functions';
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (data.user) {
          // Update the OTP record with the new user ID
          try {
            await supabase
              .from('otps')
            ...
            console.log('OTP user_id update failed, but user creation succeeded:', updateError);
          }

          // Create entry in the public.users table
          try {
            ...
            console.log('User created successfully in both auth and users table:', data.user.id);
          } catch (userInsertError) {
            console.error('Error creating user in users table:', userInsertError);
            setError('User created but failed to save profile data. Please contact support.');
            return;
          }
